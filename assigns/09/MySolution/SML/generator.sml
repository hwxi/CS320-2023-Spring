...
gfun:
('a cont ref
* unit cont ref) -> 'a): 'a generator =
let
  val ret0 =
  ref(Unsafe.cast(NONE))
  val cret =
  ref(Unsafe.cast(NONE))
  val gfun = fn(ret0, cret) =>
  let
    val () =
    generator_yield
    (Unsafe.cast(NONE), ret0, cret) in gfun(ret0, cret)
  end
in
  callcc
  (fn cc =>
   (ret0 := cc; gfun(ret0, cret))); (ret0, cret)
end
...
val gxs = generator_make_filter(gxs, fn x1 => x1 mod p1 > 0)
in
  genfun(gxs, ret0, cret)
end
in
